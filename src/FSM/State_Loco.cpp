...
void State_Loco::_loadPolicy()
{
    ...
    _hidden_size_ = _session->GetInputTypeInfo(1).GetTensorTypeAndShapeInfo().GetShape()[2];
...
void State_Loco::_action_compute()
{
    ...
    input_tensors.push_back(Ort::Value::CreateTensor<float>(memory_info, _observation.data(), _observation.size(), obs_shape.data(), obs_shape.size()));
    input_tensors.push_back(Ort::Value::CreateTensor<float>(memory_info, _h_state_.data(), _h_state_.size(), state_shape.data(), state_shape.size()));
    input_tensors.push_back(Ort::Value::CreateTensor<float>(memory_info, _c_state_.data(), _c_state_.size(), state_shape.data(), state_shape.size()));
