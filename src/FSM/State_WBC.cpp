...
void State_WBC::run()
{
    ...
    // _refer_idx is already at the correct value from enter() or previous step.
    ...
    if (_data_source == "onnx") {
        ...
        _action_compute_onnx();       // uses _refer_idx as timestep
    } else {
    ...
    // Increment _refer_idx AFTER inference (matches reference: step_count++ at end of loop).
    // First real inference uses _refer_idx=0 (set in enter()), same as reference.
    if (!_pause_flag) {
        _refer_idx++;
    }
